...
$.fn.gMapsLatLonPicker = (function() {
	...
	_self.params = {
		...
		strings : {
			markerText : "Mueve este marcador",
			error_empty_field : "No logramos encontrar las coordenadas de tu dirección",
			error_no_results : "No logramos encontrar las coordenadas de tu direcció"
		},
		...
		_self.vars.geocoder.geocode(
			...
			function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					if(posicion != null){
						posicion[0].val(posicion[1]);
						posicion[2].val(posicion[3]);
						posicion = null;
					}
				} else {
					if (!silent) {
	...
	var publicfunc = {
		...
		init : function(object) {
			...
			$(_self.vars.cssID + ".gllpSearchButton").bind("click", function() {
				performSearch( $("#"+direccion_de_busqueda).val()+" "+ciudad, false );
			});
